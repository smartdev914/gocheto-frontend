...
const useBury = (props: any) => {
    ...
    const tokenType = props.tokenType ? props.tokenType.toUpperCase() : 'SHIB'
    ...
    const approve = useCallback(
        async (amount: String = '0') => {
            try {
                console.log('ury approve', amount)
                const tx = await tokenContract?.approve(
                    buryContract?.address,
                    amount ? amount : ethers.constants.MaxUint256.toString()
                )
