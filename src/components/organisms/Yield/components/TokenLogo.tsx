...
export default function TokenLogo({ address, header = false, size, ...rest }: any) {
    ...
    if (error) {
        return (
            <Inline>
                <Image width={48} height={48} {...rest} alt={''} src={PlaceHolder} size={size} />
            </Inline>
        )
    }
    ...
    if (address === 'kashiLogo') {
        return (
            <Inline>
                <Image width={48} height={48} {...rest} alt={''} src={KashiLogo} size={size} />
            </Inline>
        )
    }
    ...
    const path = `https://raw.githubusercontent.com/SashaDesigN/assets/master/blockchains/ethereum/assets/${isAddress(
        address
    )}/logo.png`
