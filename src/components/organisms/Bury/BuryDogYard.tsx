...
import { CardHeading } from '../Home/Card'
...
import { formattedNum, formatLessThan, formatFromBalance, isAddress } from '../../../utils'
...
function tokenAddresses({ token }: { token: string }): any {
    const { chainId } = useActiveWeb3React()
    if (token?.toLowerCase() === 'shib') {
        ...
        return {
            ...
            tokenType: token.toLowerCase(),
            tokenPrice: shibPrice
        }
    }
    if (token?.toLowerCase() === 'leash') {
...
function calculate_apy(apr: any): any {
    return 0 //apr ? (Math.pow(1 + apr / 365, 365) - 1) * 100 : undefined
}

export default function BuryDogYard({ tokenName }: { tokenName: string }): JSX.Element {
    const theme = useContext(ThemeContext)
    const { account, chainId } = useActiveWeb3React()
    //const darkMode = useDarkModeManager()

    const burryshib_endpoint = graphEndpoint(chainId, graphAPIEndpoints.BURRYSHIB_GRAPH)
    const burryleash_endpoint = graphEndpoint(chainId, graphAPIEndpoints.BURRYLEASH_GRAPH)
    const burrybone_endpoint = graphEndpoint(chainId, graphAPIEndpoints.BURRYBONE_GRAPH)

    const fetcherShib = (query: any) => request(burryshib_endpoint, query)
    const fetcherLeash = (query: any) => request(burryleash_endpoint, query)
    const fetcherBone = (query: any) => request(burrybone_endpoint, query)

    const { tokenType, tokenAddress, buryTokenAddress, tokenPrice } = tokenAddresses({ token: tokenName ?? 'shib' })
        ...
        useEffect(() => {
            const fetchData2 = () => {
                if (data?.bury?.shibStakedUSD && parseFloat(bonePrice) > 0) {
                    const apr = 0 //((0.6 * parseFloat(bonePrice)) / data?.bury?.shibStakedUSD) * 277 * 24 * 30 * 12 * 100
                    const obj = {
                        apy: calculate_apy(apr),
                        text: '+ BONE APY'
                    }
                    setShibBoneApy(obj)
                }
        ...
        useEffect(() => {
            const fetchData = async () => {
                if (data?.bury?.boneStakedUSD && parseFloat(bonePrice) > 0) {
                    const apr = 0 //((0.52 * parseFloat(bonePrice)) / data.bury.boneStakedUSD) * 277 * 24 * 30 * 12 * 100
                    const obj = { apy: calculate_apy(apr), text: '+ BONE APY' }
                    setBoneBoneApy(obj)
                }
            }
                                ...
                                {tokenType === 'shib' ? (
                                    <div>
                                        <TYPE.white fontWeight={800} fontSize={'18px'} className="text">
                                            + BONE APY :{' '}
                                            {shibBoneApy?.apy ? (
                                                shibBoneApy.apy.toFixed(2) + '%'
                                            ) : (
                                                <b style={{ paddingLeft: '40px' }}>-</b>
                                            )}
                                        </TYPE.white>
