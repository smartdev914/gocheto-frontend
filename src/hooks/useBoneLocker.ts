...
const useBoneLocker = () => {
    ...
    const fetchLockedAmount = useCallback(async () => {
        if (account && claimableAmount && boneLockerContract) {
            try {
                ...
                const unClaimable: any = BigNumber.from(locked)
                    .sub(claimableAmount)
                    .toString()
                ...
                console.debug(e)
                const unClaimable: any = BigNumber.from('0')
                    .sub(claimableAmount)
                    .toString()
                setLockedAmount(unClaimable)
                return lockedAmount
    ...
    const claimAll = useCallback(async () => {
        if (account && rightCorner && boneLockerContract) {
            try {
                ...
                return boneLockerContract.estimateGas['claimAll'](rightCorner, { from: account, value: 0 })
                    .then(estimatedGasLimit => {
                        return boneLockerContract
                            .claimAll(rightCorner, {
                                from: account,
                                value: 0,
    ...
    const fetchLockInfoByLockId = useCallback(
        async lockId => {
            if (account && boneLockerContract) {
                try {
        ...
        if (account && boneLockerContract && leftCorner && rightCorner) {
            try {
                for (let lockId = parseInt(leftCorner); lockId < parseInt(rightCorner); ++lockId) {
                    ...
                    if (dateLock > datenow) return dateLock.getTime()
                }
                return 0
            } catch (e) {
                console.error(e)
