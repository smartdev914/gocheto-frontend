...
import { useShibaHelperContract } from '../hooks/useContract'
import _ from 'lodash'
import { useCallback, useEffect, useState } from 'react'
import { shibaExchange, topDog } from '../apollo/client'
import { getAverageBlockTime } from '../apollo/getAverageBlockTime'
import { liquidityPositionSubsetQuery, pairSubsetQuery, shibaSwapPoolsQuery } from '../apollo/queries'

import { ChainId, SHIBASWAP_TOPDOG_ADDRESS } from '@gocheto-dex/sdk'
...
const useShibaSwapFarms = () => {
    ...
    const TOPDOG_URI = graphEndpoint(chainId, graphAPIEndpoints.TOPDOG_GRAPH)

    const fetchAllFarms = useCallback(async () => {
        const results = await Promise.all([
            topDog(TOPDOG_URI).query({
                // results[0]
        ...
        const liquidityPosition = chainId
            ? await shibaExchange(EXCHANGE_URI).query({
                  // results[1]
        ...
        const pairAddresses = pools
            ...
            .sort()
        const pairsQuery = await shibaExchange(EXCHANGE_URI).query({
        ...
        const farms = flterPools.map((pool: any) => {
            ...
            const rewardPerBlock = ((pool.allocPoint / pool.owner.totalAllocPoint) * 0) / 1e18
