...
export const formattedNum = (number: any, usd = false) => {
    ...
    if (num > 1000) {
        return usd
            ? '$' +
                  Number(parseFloat(String(num)).toFixed(5)).toLocaleString(undefined, {
                      minimumFractionDigits: 2,
...
export const builders = {
    ...
    puppynet: (chainName = '', data: string, type: 'transaction' | 'token' | 'address' | 'block') => {
        const prefix = 'https://puppyscan.shib.io'
        switch (type) {
            case 'transaction':
                return `${prefix}/tx/${data}`
            default:
    ...
    shibarium: (chainName = '', data: string, type: 'transaction' | 'token' | 'address' | 'block') => {
    ...
    rsktestnet: (chainName = '', data: string, type: 'transaction' | 'token' | 'address' | 'block') => {
        const prefix = 'https://explorer.testnet.rootstock.io/'
        switch (type) {
            case 'transaction':
                return `${prefix}/tx/${data}`
            default:
...
const chains: ChainObject = {
    ...
    [ChainId.PUPPYNET]: {
        chainName: '',
        builder: builders.puppynet
    },
    [109]: {
        chainName: '',
        builder: builders.shibarium
    },
    [ChainId.RSK_TESTNET]: {
        chainName: 'RSK Testnet',
        builder: builders.rsktestnet
    }
}
...
export function shortenAddresMobile(address: string): string {
    const parsed = isAddress(address)
    if (!parsed) {
        throw Error(`Invalid 'address' parameter '${address}'.`)
    }
    return `${parsed.substring(42 - 4)}`
}
...
export function getShibaSwapRouterAddress(chainId?: ChainId) {
    ...
    // }
    // 0x68AC0ca4CfCc26114AB7607b112E753f77f3bc76
    return SHIBASWAP_ROUTER_ADDRESS[chainId ? chainId : ChainId.MAINNET];
}
...
export function formatMillion(number: number): string {
