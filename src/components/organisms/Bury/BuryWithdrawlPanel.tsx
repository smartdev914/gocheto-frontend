...
export default function BuryWithdrawlPanel(props: any) {
    ...
    const handleApprove = useCallback(
        async (amount: number | string = 0) => {
            try {
                const txHash = await approve(web3.utils.toWei(String(amount), 'ether'))
            ...
            <div>
                <PercentContainer>
                    <PercentAvailable style={{ float: 'left' }}>
                        Available:{' '}
                        {buryBalanceValue
                            ? buryBalanceValue.toString()
                            : buryBalanceBigInt.isLoading
                            ? 'Loading...'
                            : 0}
                    </PercentAvailable>
                    <Percent
                ...
                {!allowance || Number(allowance) < Number(input) ? (
                    <ButtonSelect
                        disabled={requestedApproval}
                        onClick={() => {
                            handleApprove(numericInput)
                        }}
                    ...
                    >
                        {!isPending ? (
                            <TYPE.white fontWeight={700} fontSize={'1rem'} color={'black'}>
                                Unstake
                            </TYPE.white>
                        ) : (
                            <TYPE.white fontWeight={700} fontSize={'1rem'} color={'black'}>
